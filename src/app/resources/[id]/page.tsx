...
function DetailStat({
  ...
  return (
    <div className="rounded-jac-md border border-jac-navy/10 bg-white/90 px-3 py-2 shadow-jac-soft">
      <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-jac-navy/55">
        <Icon className="h-3.5 w-3.5 shrink-0 text-jac-purple" aria-hidden />
        {label}
      </div>
      <div className="mt-1 text-sm font-medium text-jac-navy">{value ?? '—'}</div>
    </div>
  )
}
...
export default async function ResourceDetailPage({ params }: Props) {
  ...
  return (
    <div className="grid gap-10 pb-12 md:gap-12">
      ...
      <Card className="overflow-hidden border border-jac-purple/12 bg-gradient-to-br from-white via-jac-offwhite to-jac-pink/20 p-8 shadow-jac-medium md:p-10 lg:rounded-jac-xl">
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div className="min-w-0 max-w-3xl">
            <p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-jac-purple">
              <BookOpen className="h-4 w-4" aria-hidden />
              Jacaranda School · Learning activity
            </p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-jac-navy md:text-4xl">{resource.title}</h1>
            <p className="mt-4 text-base font-medium text-jac-purple">Use this activity to prepare for class.</p>
          </div>
        ...
        <ul className="mt-8 grid gap-3 border-t border-jac-navy/10 pt-8 text-sm text-jac-navy/85 sm:grid-cols-2">
          <li className="flex gap-2 rounded-jac-md bg-white/90 px-3 py-2 ring-1 ring-jac-purple/12">
            <PackageCheck className="mt-0.5 h-5 w-5 shrink-0 text-jac-green" aria-hidden />
            <span>Check what materials are needed before you start.</span>
          </li>
          <li className="flex gap-2 rounded-jac-md bg-white/90 px-3 py-2 ring-1 ring-jac-blue/15">
            <Info className="mt-0.5 h-5 w-5 shrink-0 text-jac-blue" aria-hidden />
      ...
      <section className="grid gap-6">
        ...
        <Card padding="p-6 md:p-8">
          {resource.description ? (
            <div className="whitespace-pre-wrap text-body leading-relaxed text-jac-navy/90">{resource.description}</div>
          ) : (
            <EmptyState
              icon={BookOpen}
        ...
        <Card padding="p-6 md:p-8">
          {resource.result_description ? (
            <div className="flex gap-3">
              <Compass className="mt-1 h-6 w-6 shrink-0 text-jac-purple" aria-hidden />
              <p className="text-base leading-relaxed text-jac-navy/90">{resource.result_description}</p>
            </div>
          ) : (
            <EmptyState
              icon={Compass}
      ...
      <section className="grid gap-6">
        <SectionHeader
          ...
          subtitle="Who this is for and what it connects to."
          icon={GraduationCap}
        />
        <Card padding="p-6 md:p-8">
          <div className="flex flex-wrap gap-2">
            {resource.subjects?.length ? (
              ...
              <span className="text-sm text-jac-navy/55">No subjects tagged.</span>
            )}
            ...
            {resource.required_materials?.length ? (
              <ul className="grid gap-3">
                {resource.required_materials.map((m) => (
                  <li
                    key={m.id}
                    className="flex gap-3 rounded-jac-md border border-jac-navy/8 bg-jac-offwhite/80 px-4 py-3 shadow-jac-soft"
                  >
                    <PackageCheck className="mt-0.5 h-5 w-5 shrink-0 text-jac-green" aria-hidden />
                    <div>
                      <div className="font-medium text-jac-navy">
                        {m.quantity ? `${m.quantity}× ` : ''}
                        {m.name}
                      </div>
                      {m.note ? <div className="mt-1 text-xs text-jac-navy/65">{m.note}</div> : null}
                    </div>
                  </li>
                ))}
              </ul>
            ...
            {resource.printable_materials?.length ? (
              <ul className="grid gap-4">
                {resource.printable_materials.map((f) => (
                  <li
                    key={f.id}
                    className="rounded-jac-md border border-jac-navy/10 bg-white px-4 py-4 shadow-jac-soft"
                  >
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <div className="font-semibold text-jac-navy">{f.title}</div>
                        {f.description ? <div className="mt-1 text-xs text-jac-navy/65">{f.description}</div> : null}
                      ...
                      <a
                        ...
                        className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-jac-purple px-4 py-2 text-sm font-medium text-white shadow-jac-soft transition hover:bg-[#6240b8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-jac-purple focus-visible:ring-offset-2 focus-visible:ring-offset-jac-offwhite"
                        target="_blank"
                        rel="noreferrer"
                      >
      ...
      <Card className="border border-jac-blue/20 bg-jac-blue/8 p-6 md:p-8">
        <p className="flex gap-3 text-sm leading-relaxed text-jac-navy/90">
          <Info className="mt-0.5 h-5 w-5 shrink-0 text-jac-blue" aria-hidden />
          <span>
            <span className="font-semibold text-jac-navy">Note:</span> public activities stay visible without an
